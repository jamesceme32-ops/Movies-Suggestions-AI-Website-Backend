...
# /tmp is used as a fast local write-through cache during a session.
# R2 is the permanent store — loaded on startup, synced after each fetch run.
...
# ══════════════════════════════════════════════
#  OMDB CACHE  (local /tmp for speed; R2 for persistence)
# ══════════════════════════════════════════════

def _warm_local_cache():
    """
    On first use, pull the cache from R2 into /tmp so the background
    thread can do fast local reads/writes during a fetch run.
    This is called once per server startup (lazily on first fetch).
    """
...
def _save_cache(cache: dict):
    """Write to /tmp (fast, called after every fetch)."""
    tmp = CACHE_PATH.with_suffix(".tmp")
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump(cache, f)
    tmp.replace(CACHE_PATH)

def _sync_cache_to_r2(cache: dict):
    """Push the local cache up to R2 for permanent storage."""
...
def fetch_omdb_background(session_id: str):
    ...
    SYNC_INTERVAL = 100   # push to R2 every 100 new fetches
    ...
    if need_fetch:
        ...
        with ThreadPoolExecutor(max_workers=16) as executor:
            ...
            for future in as_completed(futures):
                ...
                # Checkpoint: push to R2 every SYNC_INTERVAL fetches
                if fetch_count % SYNC_INTERVAL == 0:
                    _sync_cache_to_r2(cache)

    # Final R2 sync after all fetches are done
    if need_fetch:
        _sync_cache_to_r2(cache)
...
def index():
    """
    Auto-loads bookmarks from R2. No file upload needed.
    If no bookmarks have been uploaded yet, shows a prompt to go upload.
    """
    sid = get_sid()

    html_bytes = r2_storage.download_bookmarks()

    if html_bytes is None:
        # No bookmarks file in R2 yet
        return render_template("index.html", no_bookmarks=True)

    raw = parse_bookmarks(html_bytes)
...
def bookmarks():
    """
    Page for uploading / replacing the bookmarks HTML file in R2.
    The uploaded filename doesn't matter — it's always stored as bookmarks.html.
    """
    message       = None
    success       = False
